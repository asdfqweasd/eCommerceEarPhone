...
export const FormLabel = Styled.label<FormProps>`
    display:flex;
    flex-direction:column;
...
export const RadioLabel = Styled.label<RadioProps>`
    ...
    box-shadow:${(props) =>
      props["data-checked"]
        ? `inset 0px 0px 0px 2px ${COLORS.brownDark}`
        : `inset 0px 0px 0px 1px ${COLORS.grayMid} `};
    border-radius:8px;
    height:5.6rem;
    padding:0 2.4rem;
    margin-bottom:1.6rem;
...
export const PriceCost = Styled.span<PriceProps>`
    font-weight:700;
    text-align:right;
...
export const ContinueBtn = Styled.button`
    ...
    background-color:${(props) =>
      props.form ? `${COLORS.brownLight}` : `${COLORS.black}`};
    color:${COLORS.white};
    text-transform:uppercase;
    letter-spacing:.1rem;
    font-size:1.3rem;
    ...
    &:before{
        ...
        background-color:${(props) =>
          props.form ? `${COLORS.brownDark}` : `${COLORS.black}`};
